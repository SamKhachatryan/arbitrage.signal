...
impl ExchangeWSSession for BitgetExchangeWSSession {
    async fn handle_ws_session(
        ...
        let mut ping_handle = tokio::spawn(common::ping::send_ping_loop(write_arc.clone(), "Bitget"));
        ...
        tokio::select! {
            ...
            _ = cancel_token.cancelled() => {
                eprintln!("[BITGET] Cancelled - ABORTING ALL TASKS");
                ping_handle.abort();
                read_handle.abort();
                resync_handle.abort();
                drop(write_arc);
                eprintln!("[BITGET] All tasks aborted, connection dropped");
                return;
            }
        }
    }
...
async fn resync_orderbook_loop(
    ...
    tokio::select! {
        ...
        _ = cancel_token.cancelled() => {
            eprintln!("[BITGET] Resync cancelled during initial throttle for {}", pair_name);
            return;
        }
    }
    ...
    loop {
        tokio::select! {
            ...
            _ = cancel_token.cancelled() => {
                eprintln!("[BITGET] Resync cancelled for {}", pair_name);
                return;
            }
        }
